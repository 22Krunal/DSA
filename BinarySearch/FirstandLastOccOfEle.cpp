...
//This take even little more time than the linear function 
void FirstAndLastOcc(int arr[],int n,int target,int start, int end,int* mn,int* mx){
    // int start = 0 , end = n-1;
...
//For better time use bs for both first and last occurance sepratly
...
int main(){
   ...
   int mn = INT_MAX,mx = INT_MIN;
    FirstAndLastOcc(arr,n,target,0,n-1,&mn,&mx);
    if(mn == INT_MAX){
        cout << -1 << endl;
    }
    cout << mn <<" " << mx << endl;

    cout << "Faster than Previous Solution  : "  << endl;
